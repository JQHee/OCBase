...
- (void)sky_make:(NSString*)text duration:(CGFloat)duration backgroundColor:(UIColor*)color position: (CGPoint) point
{
    ...
    [textLabel sizeToFit];
    if(textLabel.frame.size.width >= (UIScreen.mainScreen.bounds.size.width-TOAST_TEXT_MARGIN_LEFT_RIGHT*2))
    {
        CGRect rect = textLabel.frame;
        rect.size.width = (UIScreen.mainScreen.bounds.size.width-TOAST_TEXT_MARGIN_LEFT_RIGHT*2);
...
- (void)sky_make:(NSString*)text duration:(CGFloat)duration position:(CGPoint)point
{
   ...
   backgroundColor:[UIColor blackColor]
          position:CGPointMake(UIScreen.mainScreen.bounds.size.width/2, TOAST_POSITION_Y)];
}
